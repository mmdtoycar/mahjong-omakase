...
function parseHand(
  ...
  for (const token of tokens) {
    if (token.startsWith('chi:')) {
      ...
      melds.push({
        type: 'shun',
        tiles: [new Tile(suit, rank), new Tile(suit, rank + 1), new Tile(suit, rank + 2)],
        isOpen: true,
      })
...
describe('Guobiao Benchmarks - Special Scoring Rules', () => {
  ...
  test('Rule 8', () => {
    expectFans(
      'p5 p5 p5 p6 p6 p6 p7 p7 p7 s5 s5 s5 s6 s7',
      ['一色三同顺', '全带五', '不求人', '平和', '喜相逢', '缺一门'],
      { isSelfDraw: true }
    )
  })
  ...
  test('Rule 15', () => {
    expectFans('pung:s2 p1 p2 p3 p4 p5 p6 p7 p8 p9 z1 z1', ['清龙', '单钓将', '缺一门', '自摸'], {
      isSelfDraw: true,
      gangShang: true,
    })
  })
  ...
  test('Rule 17', () => {
    expectFans('gang:s2 z1 z1 p1 p2 p3 p4 p5 p6 p7 p8 p9', ['清龙', '缺一门', '杠上开花', '明杠'], {
      isSelfDraw: true,
      gangShang: true,
    })
  })
  ...
  test('Rule 18', () => {
    expectFans('gang:s2 z1 z1 p1 p2 p3 p4 p5 p6 p7 p8 p9', ['清龙', '缺一门', '明杠', '妙手回春'], {
      isSelfDraw: true,
      lastTile: true,
    })
  })
  ...
  test('Rule 20', () => {
    expectFans('gang:s2 z1 z1 p1 p2 p3 p4 p5 p6 p7 p8 p9', ['清龙', '缺一门', '明杠', '妙手回春', '杠上开花'], {
      isSelfDraw: true,
      gangShang: true,
      lastTile: true,
    })
  ...
  test('Rule 21', () => {
    expectFans('gang:s2 z1 z1 p1 p2 p3 p4 p5 p6 p7 p8 p9', ['清龙', '缺一门', '明杠', '海底捞月', '抢杠和'], {
      gangShang: true,
      lastTile: true,
    })
  })
  ...
  test('Rule 24', () => {
    expectFans(
      'p1 p1 p1 p9 p9 p9 m1 m1 m1 z1 z1 z1 s5 s5',
      ['四暗刻', '双同刻', '圈风刻', '门风刻', '幺九刻', '幺九刻', '幺九刻', '单钓将'],
      { quanfeng: 1, menfeng: 1 }
    )
  })
  ...
  test('Rule 27', () => {
    expectFans(
      's1 s2 s3 s7 s8 s9 m1 m2 m3 m7 m8 m9 p6 p6',
      ['单钓将', '喜相逢', '平和', '喜相逢', '老少副', '门前清'],
      {}
    )
  })
  ...
  test('Rule 29', () => {
    expectFans(
      'pung:s8 s6 s6 s2 s3 s4 s2 s3 s4 s3 s4 s2',
      ['绿一色', '一色三节高', '碰碰和', '双暗刻', '清一色', '断幺'],
      {}
    )
  })
