 ...
 * Shared across all game modes (Guobiao, Riichi, Dongbei)
 */
...
export class Tile {
  ...
  get isWind(): boolean {
    return this.suit === 'z' && this.rank >= 1 && this.rank <= 4
  }
  get isDragon(): boolean {
    return this.suit === 'z' && this.rank >= 5 && this.rank <= 7
  }
  get isGreen(): boolean {
    return (this.suit === 's' && [2, 3, 4, 6, 8].includes(this.rank)) || (this.suit === 'z' && this.rank === 6)
  }
