    ...
    return (
      <td className="cell-rank">
        <div className="cell-player-box">
          ...
          <span className={`cell-player-stats ${p.totalScore > 0 ? 'score-positive' : p.totalScore < 0 ? 'score-negative' : ''}`} style={{ fontWeight: 700 }}>
              ...
              {sessions.map((s) => (
                <tr key={s.id} onClick={() => navigate(`/session/${s.id}`)} style={{ cursor: 'pointer' }}>
                  ...
                  <td className="cell-date">
                    {new Date(s.createdAt).toLocaleDateString([], { month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' })}
                  </td>
                  <td>
                    <span className={`cell-status-badge badge ${s.status === 'IN_PROGRESS' ? 'badge-progress' : 'badge-completed'}`}>
