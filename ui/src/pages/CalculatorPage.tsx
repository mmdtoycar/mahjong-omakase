...
const modes: Mode[] = [
  ...
  },
  {
    ...
    isDisabled: (c, m, t) => {
      ...
      return (
        all.filter((x) => x.equals(t1)).length >= 4 ||
        all.filter((x) => x.equals(t2)).length >= 4 ||
        all.filter((x) => x.equals(t3)).length >= 4
      )
    },
...
const TileComponent: React.FC<{
  ...
  return (
    <div
      className={`calc-tile-container ${size} ${isSelectable && !disabled ? 'selectable' : ''} ${
        disabled ? 'disabled' : ''
      } ${className}`}
            ...
            <div className="extra-options-row">
              ...
              <div className="mini-option flowers">
                ...
                <select
                  ...
                  onChange={(e) => setOptions({ ...options, huaCount: parseInt(e.target.value) })}
                >
          ...
          <div className="tile-picker-card">
            <div className="mode-selector-container">
              <div className="mode-group">
                ...
                {modes
                  .filter((m) => (m.name === 'normal' || m.name.startsWith('an-')) && m.name !== 'an-gang')
                  .map((m) => (
                    <button
                      key={m.name}
                      onClick={() => setMode(m)}
              ...
              <div className="mode-group">
                ...
                {modes
                  .filter((m) => (!m.name.startsWith('an-') && m.name !== 'normal') || m.name === 'an-gang')
                  .map((m) => (
                    <button
                      key={m.name}
                      onClick={() => setMode(m)}
                ...
                <button className="mode-btn btn-reset" onClick={onResetClick}>
                  重置
                </button>
              </div>
            </div>
              ...
              <div className="hand-group animate-right">
                <div className="tiles-row">
                  {displayConcealed.map((tile, i) => (
                    <TileComponent key={i} tile={tile} onClick={() => onHandTileClick(tile)} isSelectable />
                  ))}
                </div>
              ...
              <div className="ting-tiles">
                {tingResults.length > 0 ? (
                  [...tingResults]
                    ...
                    .map(
                      (
                        res: { tile: Tile; score: number; fans: { name: string; score: number; count?: number }[] },
                        i: number
                      ) => (
          ...
          {(tingResults || currentCount === 14) && (
            <div className="winning-options-section animate-up">
              ...
              <div className="options-grid">
                <button
                  className={`opt-btn ${options.isSelfDraw ? 'active' : ''}`}
                  onClick={() => setOptions({ ...options, isSelfDraw: !options.isSelfDraw })}
                >
                  自摸{' '}
                  <span className="btn-hint">
                    +{options.isSelfDraw && melds.filter((m) => m.isOpen).length === 0 ? '4 (不求人)' : '1'}
                  </span>
                </button>
                <button
            ...
            <div className="result-header">
              ...
              <div className="hu-status">
                ...
                <p style={{ opacity: 0.6, margin: '4px 0 0 0' }}>
                  {huResult.totalScore >= 8 ? '符合国标8番起和点数' : '还差 ' + (8 - huResult.totalScore) + ' 番'}
                </p>
              </div>
            </div>
